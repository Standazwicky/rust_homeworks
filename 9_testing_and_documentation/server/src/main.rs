       ...
       match message {
          MessageType::Login(username) => {
              if user_exists(&db_pool, &username).await? {
                  clients.lock().await.insert(addr, (stream.clone(), username.clone()));
               ...
               let error_message = MessageType::Error("User does not exist. You can create new user by \n 
               .register <username> 
          ...
          MessageType::Register(username) => {
              if register_user(&db_pool, &username).await.is_ok() {
                  ...
                  let welcome_message = MessageType::Text(format!("User {} registered successfully", username));
                  send_message(stream.clone(), &welcome_message).await?;
                  break;
              } else {
          ...
          _=> {
            let error_message = MessageType::Error("Please login or register. \n .login <username> \n or \n .register <username>".to_string());
            send_message(stream.clone(), &error_message ).await?;
          }
       }
    }
...
async fn read_message(stream: Arc<Mutex<TcpStream>>) -> Result<MessageType> {
    let mut len_bytes = [0u8; 4];
    {
        ...
        stream.read_exact(&mut len_bytes).await.context("Failed to read message lenght")?;
    ...
    match message {
        ...
        MessageType::Image(data) => {
            ...
            task::spawn_blocking(move || { 
                let image = image::load_from_memory(&data_clone).context("Failed to load image from memory")?;
                let mut output_file = std::fs::File::create(&filename_clone).context("Failed to create image file")?;    
        ...
        MessageType::File(name, data) => {
            ...
            fs::write(&filename, &data).await.context("Failed to save file")?;
...
async fn listen_and_accept(address: &str, db_pool: Pool<Postgres>) -> std::io::Result<()> {
    ...
    loop {
        ...
        clients.lock().await.insert(addr, (stream.clone(), String::new()));
...
async fn ensure_directories_exist() -> Result<()> {
    ...
    for path in &paths {
        if !Path::new(path).exists() {
            fs::create_dir(path).await.context(format!("Failed to create directory {}", path))?;
        }
    }
    Ok(())
}
